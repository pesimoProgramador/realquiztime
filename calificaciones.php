...
if ($notas != null) {
    ...
    $params = array('itemname'=>$realquiztime->name, 'idnumber'=>$realquiztime->id);
    for ($i = 0; $i < $notas_length; $i++) {
        $grade = new stdClass();
        $grade->userid   = $students_array[$i]->id;
        $grade->rawgrade = $notas[$i];
        if ($notas[$i] != "") {
            grade_update('mod/realquiztime', $realquiztime->course, 'mod', 'realquiztime', $realquiztime->id, 0, $grade, $params);
        }
    }
}
...
$grading_info = grade_get_grades($course->id, 'mod', 'realquiztime', $realquiztime->id, $usersId);
...
$PAGE->set_url('/mod/realquiztime/calificaciones.php', array('id' => $cm->id));
...
if (!$alumno) {
    ...
    <script type="text/javascript" src="<?php echo $CFG->wwwroot?>/mod/realquiztime/js/zingchart.min.js"></script>
    ...
    <script type="text/javascript" src="<?php echo $CFG->wwwroot?>/mod/realquiztime/js/calificaciones.js"></script>
    <?php
    ...
    <div id="divInterfaz">
        ...
        <div id="divNotas">
            <form id="formTable" method="post" action="calificaciones.php">
            ...
            <table>
                <tr>
                    ...
                    for($i = 0; $i < $notas_length; $i++) {
                        ...
                        echo '<td>'; echo '<textarea class="textarea" id='; echo 'textarea'.$students_array[$i]->email; echo ' form="formTable" name="text[]"></textarea></td>';
                        echo '</tr>';
                    }
        echo'
            </table>
